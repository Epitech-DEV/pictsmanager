...
mixin EventEmitterMixin {
  ...
  void onPipe<T extends IEvent>(PipeListener<T> listener);

  /// Remove a pipe listener from a type of event
  void removePipeListener<T extends IEvent>(PipeListener<T> listener);
  ...
  FutureOr<T> pipe<T extends IEvent>(T event);
}
...
class EventEmitter with EventEmitterMixin {
  ...
  /// If one of the pipes throws an exception, the rest of the pipes will not be called
  @override
  FutureOr<T> pipe<T extends IEvent>(T event) async {
